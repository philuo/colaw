...
describe('SSE framing and provider failures', () => {
  async function read(text: string, bytewise = false) {
    // The framer consumes decoded text: Bun 1.4's `response.textStream()` owns
    // byte→UTF-8 decoding, so this pins arbitrary chunk boundaries mid-frame.
    let offset = 0
    const body = new ReadableStream<string>({ pull(controller) {
      if (offset === text.length) { controller.close(); return }
      const end = bytewise ? offset + 1 : text.length
