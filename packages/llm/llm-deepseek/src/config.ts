...
// The full harness vocabulary: this provider dispatches only the wire members
...
function resolveModels(models: readonly DeepSeekCatalogModel[] | undefined): DeepSeekCatalogModel[] {
  ...
  return (models ?? DEFAULT_MODELS).map((model) => {
    ...
    if (inputModalities.some(modality => !MODEL_MODALITIES.includes(modality))) {
      throw new Error(
        `llm-deepseek: catalog model "${model.id}" inputModalities must contain only ${MODEL_MODALITIES.map(m => `"${m}"`).join(', ')}`,
      )
    }
