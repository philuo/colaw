/**
 * Translate OpenAI chat-completions SSE payloads with one stateful harness
 * block per content, reasoning, or tool call index. An empty initial
...
/**
 * Map wire usage fields. Both OpenAI prompt-cache reads and DeepSeek-dialect
 * cache hits make `prompt_tokens` an INCLUDE-IVE count
 * (`prompt_tokens = uncached + cached`); the harness TokenUsage convention is
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      // Reasoning text first (DeepSeek-dialect gateways emit it before text;
      // official OpenAI never does). The empty-string first chunk must not
      // open a block.
