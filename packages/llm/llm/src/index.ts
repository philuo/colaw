...
export * from './sse-frames.ts'
export * from './legacy-pi-ai.ts'
...
export function assertUsableApiKey(raw: string, pkg: string, ref: string): string {
  ...
  // The Models page is named as the writer: a credential reaches a request
  // only through the managed store, never from the launching environment.
  throw new LlmError(
