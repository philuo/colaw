 ...
 * reports each wrap's enforcement and stderr classification facts. Missing or
 * unusable confinement fails closed rather than returning the original argv.
 *
 * This fork targets macOS arm64; the Windows ACL restricted-token runner chain
 * (and its grant packages) were removed with the rest of the win32 compat
 * surface.
 ...
 * `sandbox_init` refuses it). A missing `sandbox-exec` fails the spawn and
 * probes `unusable`, exactly like the other rungs' absent binaries. Apple
 * marks the CLI deprecated but ships it on every macOS; if it ever
 * disappears, this probe is what fails closed.
 */
