...
function validateTarget(target: ImageRequestTarget): void {
  checkedInteger(target.width, 'Image request target width')
  checkedInteger(target.height, 'Image request target height')
...
/**
 * Complete deterministic identity for one attachment and route-owned request target.
 * @param attachment - provider-independent durable normalized attachment reference.
 * @param target - route-owned target dimensions and byte budget.
...
async function createRequestImage(
  ...
  // The caller resolves the route's target from the source size, so a target at
  // or above the source keeps the original bytes whenever they already fit. The
  // resize stage also runs `fit: 'inside'` with `withoutEnlargement`, so an
  // oversized target can never upscale the source.
  ...
  const encodedVersion = await encodeFirstWithinLimit(
    encodingLadder(pipeline(attachment, target.width, target.height), hasAlpha),
    target.maxBytes,
  )
 ...
 * @param target - exact route request-image target (dimensions and byte budget).
