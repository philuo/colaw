...
// The fork's only PTY backend is Bun.Terminal (see bun-pty-adapter.ts):
// the child gets no controlling terminal, so macOS `ps -o tpgid=` always
// reports 0 and the inspector must conservatively answer "unknown" instead of
// idle/busy. Prompt-hook gating that depends on the kernel foreground group
// cannot be exercised on this backend; startup-file integration still can.
const controllingTerminalActivity = (globalThis as { Bun?: unknown }).Bun === undefined

describe.skipIf(process.platform === 'win32' || !existsSync('/bin/zsh'))('Zsh terminal activity', () => {
  describe.skipIf(!controllingTerminalActivity)('foreground activity via controlling terminal', () => {
