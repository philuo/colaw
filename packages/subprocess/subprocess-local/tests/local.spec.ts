...
describe('LocalSubprocessRuntime', () => {
  ...
  it('validates terminal allocation inputs before allocating a PTY', async () => {
    ...
    const base: SubprocessTerminalSpawnSpec = {
      argv: ['bash'], cwd: process.cwd(), rows: 24, cols: 80, graceMs: 10, terminalType: 'dumb',
    }
  ...
  it('releases a terminal after top-level exit reaches quiescence', async () => {
    ...
    vi.resetModules()
    vi.doMock('../src/bun-pty-adapter.ts', () => ({ spawn: () => terminal }))
    ...
    try {
      ...
      const handle = await ctx.subprocess.spawnTerminal({
        argv: ['shell'], cwd: process.cwd(), rows: 24, cols: 80, graceMs: 1, terminalType: 'dumb',
      })
      ...
      vi.doUnmock('../src/bun-pty-adapter.ts')
