...
- **只有 pi-ai 路由可以手工声明**：自定义提供方卡片写入 `llm-provider`——唯一一个其 profile 描述整个提供方的 namespace。`llm-deepseek` 路由是组合面的事实，不是本页能创建的东西。
