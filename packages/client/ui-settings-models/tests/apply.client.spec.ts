...
describe('ui-settings-models apply', () => {
  ...
  it('registers the models nav entry for declarations before or after apply', async () => {
    ...
    // Keys are configured on the page itself (the provider editors write the
    // credential store); no startup onboarding dialog exists to assert.
  ...
  it('accepts extension entries under the declared seats and cascades them with the declarer', async () => {
    ...
    const disposeCard = b.slots.register(
      { name: 'settings.models.provider-card', key: 'llm-provider' } as never,
      () => null,
    )
...
describe('pushed invalidations', () => {
  it('ignores invalidations before the page ever loaded', async () => {
    ...
    b.remote.emit('settings/document-updated', ['llm-provider', 1])
  ...
  it('routes pushed credential invalidation into the loaded page join', async () => {
    const b = await bench()
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
