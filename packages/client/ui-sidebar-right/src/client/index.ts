...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => {
    ...
    // Upstream nests the panel seat under a session-scoped root: `rightbar` holds
    // one `rightbar.session` child, and the panel seat lives there. We keep that
    // structure. The pinned toggle is not a seat of its own — `RightbarSeat`
    // draws it beside the panel from the same store — so there is deliberately
    // no `conversation.session.header.corner` registration here.
