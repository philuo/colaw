...
import { apply, OFD_HIFI_BODY_ID, OFD_TEXT_BODY_ID } from '../src/client/ofd/index.ts'
...
describe('OFD registration', () => {
  it('registers both bodies over .ofd and removes everything on dispose', async () => {
    ...
    try {
      ...
      expect(previews.candidates('document.OFD').map(entry => entry.id)).toEqual([OFD_TEXT_BODY_ID, OFD_HIFI_BODY_ID])
      ...
      expect(previews.candidates('a.ofd')[0]!.title()).toBe(en.textTitle)
      expect(previews.candidates('a.ofd')[1]!.title()).toBe(en.hifiTitle)
      expect(dictionaries.get('sidebarOfd')).toEqual({ zh, en })
      // One keyed body per implementation, both reading the same dictionary.
      expect(entries.map(entry => entry.key)).toEqual([OFD_TEXT_BODY_ID, OFD_HIFI_BODY_ID])
      expect(new Set(entries.map(entry => entry.component)).size).toBe(2)
