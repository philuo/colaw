...
PNG、JPEG、GIF、WebP、BMP、ICO 和 SVG 通过 Blob URL 在 `<img>` 静态图片上下文中渲染。默认姿态是纯 CSS 的 contain 收纳（在硬性定界的视口内）——整图居中完整可见由布局引擎结构性保证，不存在会被未稳定的布局污染的"测量式适配比例"。细长图的显示宽度不低于面板宽度的一半；此时高度按设计超出面板，用普通滚轮或拖拽上下平移浏览，体验与滚动长文档一致。⌘/Ctrl+滚轮（触控板捏合手势）以指针为锚点缩放，系数与手势像素幅度成正比（滚轮一档约 ×1.35，单次事件钳制在 ×0.5–×2）；双击在适配与 2 倍之间切换；下限 k=1 恰好就是完整视图，缩小永远落回整图。比例徽标显示原始像素缩放并点击复位。SVG 标记绝不进入应用 DOM 或 iframe，因此其中的脚本无法执行，也无法访问父页面。替换或卸载图片会撤销其 Blob URL。
