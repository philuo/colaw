...
PNG、JPEG、GIF、WebP、BMP、ICO 和 SVG 通过 Blob URL 在 `<img>` 静态图片上下文中渲染。默认姿态是 `object-fit: contain` 铺满硬性定界的视口——整图居中完整可见、宽高比由渲染引擎结构性保证，右侧栏宽度变化只做等比重适配，绝不拉伸；也不存在会被未稳定布局污染的"测量式适配比例"。静止时不附加任何 transform，浏览器按布局尺寸原生栅格化，画面保持锐利。⌘/Ctrl+滚轮（触控板捏合手势）以指针为锚点缩放，系数与手势像素幅度成正比（捏合约快 1.75 倍，滚轮一档触达 ×2 单事件上限）；放大超出面板后可用普通滚轮或拖拽平移；双击在适配与 2 倍之间切换；下限 k=1 恰好就是完整视图，缩小永远落回整图。比例徽标显示原始像素缩放并点击复位。SVG 标记绝不进入应用 DOM 或 iframe，因此其中的脚本无法执行，也无法访问父页面。替换或卸载图片会撤销其 Blob URL。
