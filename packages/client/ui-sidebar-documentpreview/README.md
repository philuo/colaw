...
PNG, JPEG, GIF, WebP, BMP, ICO, and SVG render through Blob URLs in an `<img>` static-image context. The pane is hard-bounded (the viewport overlays the frame absolutely), so even very tall images measure the true pane and open fully contained, centered on both axes and never upscaled past their natural pixels. ⌘/Ctrl+wheel (the trackpad pinch gesture) zooms toward the pointer with a factor proportional to the gesture's pixel magnitude — a wheel notch is a comfortable step while a pinch's rapid small deltas compose into one smooth continuous zoom (one event is clamped to ×0.5–×2) — a drag pans while zoomed in with edge clamping, a double-click toggles between fit and a 2× close-up at the point, and the scale badge reports natural-pixel zoom and resets to fit on click. SVG markup never enters the application DOM or an iframe, so its scripts cannot execute or reach the parent page. Replacing or unmounting the image revokes its Blob URL.
