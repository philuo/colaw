...
PNG, JPEG, GIF, WebP, BMP, ICO, and SVG render through Blob URLs in an `<img>` static-image context. The resting posture is `object-fit: contain` filling a hard-bounded viewport — the whole image is centered, complete, and proportion-exact by construction; a pane resize re-contains it without ever stretching, and no measured "fit scale" exists that a mid-settle layout could corrupt. No transform is applied at rest, so the image rasterizes at its laid-out size and stays sharp. ⌘/Ctrl+wheel (the trackpad pinch gesture) zooms toward the pointer with a factor proportional to the gesture's pixel magnitude (a pinch runs ~1.75× faster; one event is clamped to ×0.5–×2), a plain wheel or drag pans while zoomed past the pane, a double-click toggles fit ↔ 2×, and the floor k = 1 is exactly the complete view, so zooming out always lands back on the whole image. Past the layer budget (4096 device pixels on the displayed longer axis) the viewer switches to a deep-zoom canvas: a pane-sized backing store samples the visible region at display resolution, so every zoom level renders sharp with bounded memory (Preview.app's approach); zooming back under the budget restores the transform path. Image text is recognized silently in the background (Apple Vision through the desktop host's `osascript` bridge — no extra binary) and laid over the image as a transparent, selectable text layer: no button or chrome, the operator selects and copies image text like a document, and the layer shares the gesture transform at every zoom level, deep-zoom included. Repeated opens answer from a small per-path cache; SVG and thumbnail-sized images are skipped; failures stay silent. The scale badge reports natural-pixel zoom and resets to fit on click. SVG markup never enters the application DOM or an iframe, so its scripts cannot execute or reach the parent page. Replacing or unmounting the image revokes its Blob URL.
...
- **Byte-view scroll state is not restored.** PDF and HTML can return to the top when their renderer remounts or reloads; HTML iframe scrolling belongs to its opaque browsing context. Image zoom and pan are view-local and also reset with a remount.
