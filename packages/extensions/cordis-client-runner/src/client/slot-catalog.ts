...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.composer.contextPanel',
    kind: 'list',
    scope: 'session',
    summary: 'Sections inside the composer context meter\'s open panel.',
    doc: 'Sections inside the composer context meter\'s open panel.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [
      'client-ui-chat ComposerStatsPanel id \'stats\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.composer.contextPanel\', () => ctx.slots.register(\n      { name: \'conversation.composer.contextPanel\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-general IdentityRow id \'identity\'',
  ...
  {
    key: 'settings.plugins.builtin',
    kind: 'keyed',
    scope: 'root',
    summary: 'One built-in service\'s credential card inside the 内置插件 tab, keyed by the service (`pkulaw` / `qcc` / `firecrawl` / `mineru`).',
    doc: 'One built-in service\'s credential card inside the 内置插件 tab, keyed by\nthe service (`pkulaw` / `qcc` / `firecrawl` / `mineru`). The keys live\nonly in the credentials domain, so unlike `settings.plugin.item` the key\nnames a service, not a settings namespace.',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'settings.plugins.tab\' (client-ui-settings-plugins), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-plugins function BuiltInCardBound(builtInProps) { return createEl…',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'settings.plugins.builtin\', () => ctx.slots.register(\n      { name: \'settings.plugins.builtin\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-settings-plugins/src/client/slot-contract.ts:26',
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-plugins BuiltInServicesTab id \'builtin\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-settings-general AboutSettingsSection id \'about\'',
      'client-ui-settings-general GeneralSection id \'general\'',
      'client-ui-settings-models ModelsSection id \'models\'',
      'client-ui-settings-plugins PluginsSettingsSection id \'plugins\'',
      'client-ui-workspace TrashSettingsSection id \'trash\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-sidebar-documentpreview EpubBody',
      ...
      'client-ui-sidebar-documentpreview MediaBody',
      'client-ui-sidebar-documentpreview OfdTextBody',
      'client-ui-sidebar-documentpreview OfdHifiBody',
      'client-ui-sidebar-documentpreview Body',
