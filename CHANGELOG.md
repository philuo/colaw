...
- **图片默认完整显示改为 CSS 结构性保证**：不再由 JS 测量面板计算"适配
  ...
  回完整视图）。默认姿态现在完全交给浏览器 CSS 的 contain
  （`max-width/max-height: 100%` + 绝对定位定界视口），整图居中完整可见
  由布局引擎本身保证；缩放只是在 CSS 适配之上叠加 transform，下限
  k=1 恰好就是完整视图——捏合缩小永远能回到整图。
- **宽度下限 50%**：细长图（长截图等）contain 后会窄成一条，现在显示
  宽度不低于容器宽度的一半；此时高度按设计超出面板，用普通滚轮或拖拽
  上下平移浏览，体验与滚动长文档一致。
- **丝滑度提升**：缩放灵敏度 0.0025 → 0.003（滚轮一档约 1.35 倍），
  捏合缩放更跟手；普通滚轮在图片超出面板时直接平移（不再只是缩放
  修饰键的附带功能）。
- 徽标百分比、可平移态与缩放后的平移钳制均改为读取布局后的真实盒子，
  与渲染不可能不一致。
