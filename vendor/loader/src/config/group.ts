    ...
    await Promise.all(ids.map(async (id) => {
      if (newMap[id]) {
        await this.create(newMap[id]).catch((error) => {
          // Colaw fork: keep a loud, greppable stderr line as well — host boot
          // failures were previously only visible through the logger sink.
          console.error('[loader-apply-failure]', error instanceof Error ? `${error.name}: ${error.message}` : String(error))
