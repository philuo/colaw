...
const standardReleaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/(?!electrobun-host$)[^/]+|vendor\/[^/]+)$/
/** Local application assembled by Hutch rather than published to npm. */
const localApplicationDirectory = 'apps/electrobun-host'
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (dir.startsWith('apps/') && dir !== localApplicationDirectory && manifest.name?.startsWith('@deepseek-ai/')) {
