...
let rep = NSBitmapImageRep(bitmapDataPlanes: nil, pixelsWide: 920, pixelsHigh: 720, bitsPerSample: 8,
  samplesPerPixel: 4, hasAlpha: true, isPlanar: false, colorSpaceName: .deviceRGB, bytesPerRow: 0, bitsPerPixel: 0)!
rep.size = NSSize(width: 460, height: 360)
NSGraphicsContext.saveGraphicsState()
NSGraphicsContext.current = NSGraphicsContext(bitmapImageRep: rep)
NSColor(calibratedWhite: 0.958, alpha: 1).setFill()
NSBezierPath(rect: NSRect(x: 0, y: 0, width: 460, height: 360)).fill()
...
for (x, y, rot, draw) in shapes {
  ...
  let sx = CGFloat(x*scale)+16, sy = CGFloat(y*scale)+16
  NSGraphicsContext.current?.cgContext.translateBy(x: sx+16, y: sy+16)
  NSGraphicsContext.current?.cgContext.rotate(by: CGFloat(rot))
  NSGraphicsContext.current?.cgContext.translateBy(x: -16, y: -16)
  ink.setStroke(); ink.setFill()
  draw(NSRect(x: sx-16, y: sy-16, width: 32, height: 32))
...
let ts = title.size()
title.draw(at: NSPoint(x: (460-ts.width)/2, y: 196))
let arrow = NSBezierPath()
arrow.move(to: NSPoint(x: 168, y: 128))
arrow.curve(to: NSPoint(x: 286, y: 128), controlPoint1: NSPoint(x: 208, y: 104), controlPoint2: NSPoint(x: 252, y: 104))
arrow.lineWidth = 6; arrow.lineCapStyle = .round; NSColor(calibratedWhite: 0.42, alpha: 1).setStroke(); arrow.stroke()
let head = NSBezierPath()
head.move(to: NSPoint(x: 286, y: 148)); head.line(to: NSPoint(x: 300, y: 130)); head.line(to: NSPoint(x: 280, y: 121)); head.close()
