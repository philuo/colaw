...
async function emitHostBundle(closure: Closure): Promise<void> {
  const text = await buildUnit(hostEntry, 'bun', 'esm', externalsFor(closure, '@deepseek-ai/dsh-electrobun-host'))
  if (text === undefined) {
    console.error('pack-stable-app: bundling the host entry failed')
    process.exit(1)
  }
  writeFileSync(join(appResourcesApp, 'bun', 'index.js'), text)
}
