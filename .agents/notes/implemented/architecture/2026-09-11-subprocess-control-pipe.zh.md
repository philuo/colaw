...
部分由[《subprocess 控制通道改走 IPC》](2026-09-21-bun-ipc-control-channel.zh.md)取代：专用通道的决策及其子进程 API 仍然有效，继承描述符这一传输方式不再有效。
...
普通 subprocess 请求可以设置 `stdio.control: 'pipe'`，并通过 `handle.control` 收到原始 `Duplex`。目标通过 `@deepseek-ai/dsh-subprocess/control` 打开继承的控制通道。提供方拥有环境标记；子进程辅助函数会消费它。消费方负责有界分帧、消息校验、背压和端点关闭。标准输出收集与受管范围生命周期保留现有语义。提供方独立跟踪打开的控制端点，直到它们关闭，包括受管范围退出之后；销毁时先尝试受管范围拆卸，再关闭剩余端点。

本地提供方通过 stdio 槽 3 的 IPC 通道承载该通道，Node 与 Bun 都能确定性地投递；远程提供方以自己的传输保留相同的子进程 API。该通道不授予任何宿主能力：子进程仍不可信，每次宿主工具请求都需要通常的分发与审批检查。
