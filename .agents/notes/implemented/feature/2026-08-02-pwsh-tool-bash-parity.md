...
- **Out of scope, unchanged**: persistent PTY shells (backends are Linux/macOS-only; ConPTY is roadmap work). The pwsh tool carries the sandbox denial rendering and the same-turn `sandbox_permissions` escalation surface. The pwsh-specific terminal card with an exit pill shipped separately in the [pwsh UI presentation matches bash](../../archived/feature/2026-08-05-pwsh-ui-bash-parity.md) decision.
