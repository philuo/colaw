...
async function main() {
  ...
  // Anchor selection: DSH_E2E_ANCHOR overrides the repo default. The packaged
  // app's main process resolves its install anchor to the bundled closure
  // (Resources/app/bundles), so pointing DSH_E2E_ANCHOR at a build's bundles
  // package.json reproduces the packaged boot path exactly.
  const envAnchor = process.env.DSH_E2E_ANCHOR
  const cliPackageJson = envAnchor
    ?? join(fileURLToPath(new URL('../../../..', import.meta.url)), 'apps', 'cli', 'package.json')
  console.log(`  Install anchor: ${cliPackageJson}`)
  const profile = loadProfile('dsh e2e', 'web', cliPackageJson)
  ...
  const ctx = await boot('dsh e2e', rootConfig, structuredClone(patches), (hostCtx) => {
    ...
    // --no-open by default (the test has no browser surface); override with
    // DSH_E2E_NO_OPEN=0. DSH_E2E_PORT lets the test bind a free port when
    // another dsh instance already owns 3080.
    const args: string[] = []
    if (process.env.DSH_E2E_NO_OPEN !== '0') args.push('--no-open')
  ...
  if (indexText.includes('__DSH_BOOT__')) {
    ...
    const bootMatch = indexText.match(/__DSH_BOOT__\s*=\s*(\{.*?\});/s)
    if (bootMatch) {
      try {
        const boot = JSON.parse(bootMatch[1])
        const modules = Array.isArray(boot.modules) ? boot.modules.length : 'n/a'
        const bundles = Object.keys(boot.bundles ?? {}).length
        console.log(`  __DSH_BOOT__ modules: ${modules}, bundles: ${bundles}`)
      } catch { console.log('  __DSH_BOOT__ (parse skipped)') }
    }
  } else {
  ...
  // 6. /plugins/ bundle endpoint — use the exact URL the HTML preloads
  // (HTML-entity-decoded: the document escapes `&rev=` as `&amp;rev=`).
  console.log('\n[6/7] Testing /plugins/ bundle endpoint...')
  const preloadMatch = indexText.match(/\/plugins\/\?\?@deepseek-ai\/dsh-client-modules\/client\.js[^"']*/)
  const pluginPath = preloadMatch?.[0].replaceAll('&amp;', '&')
  if (pluginPath === undefined) {
    throw new Error('No /plugins/??@deepseek-ai/dsh-client-modules/client.js preload found in HTML')
  }
  const pluginRes = await fetch(`${baseUrl}${pluginPath}`, {
    headers: { 'Cookie': setCookie },
  })
  console.log(`  GET ${pluginPath} → ${pluginRes.status}, ${pluginRes.headers.get('content-type')}`)
  if (pluginRes.status === 200) {
    console.log('  ✅ Plugin bundle endpoint works')
  } else {
    throw new Error(`Plugin bundle preload failed with ${pluginRes.status}: ${(await pluginRes.text()).slice(0, 120)}`)
  }
