...
try {
  // fs-ext is no longer a dependency of this product — the file lease uses the
  // runtime's own flock through Bun FFI — so the payload carries no such module
  // and the smoke must not require one.
  checkKoffi()
  await checkSharp()
  checkHtml()
  await checkPty()
