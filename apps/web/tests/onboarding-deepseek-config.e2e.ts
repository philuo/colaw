...
// credential is absent, and the inline key write lands in an isolated harness
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // nothing. Holding settings/describe widens that window from loopback RTT
    // scale to a deterministic hundreds of milliseconds, removing all timing
    // dependence from the sampler assertions below.
    //
    ...
    // because no later scenario in this file legitimately shows the takeover,
    // and only this test reads __takeoverSightings.
  ...
  it('keeps the fixture inventory closed', async () => {
    await assertFixtureInventory(
      SNAPSHOT_DIR,
      ['missing.expected.md', 'models.expected.md', 'default-models.expected.md'],
    )
  })
})
