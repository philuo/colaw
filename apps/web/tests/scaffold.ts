...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  // real ~/.colaw document or credential file.
  ...
  // session is created. Without this a developer's real ~/.colaw/skills silently
  ...
  const overlayPatches: PatchOptions[] = [
    ...
    // root away: a developer's own `~/.colaw/.agent-presets` must not be able
    // to change a golden.
    {
      id: 'agent-presets',
      config: {
    ...
    // the owned temp world so ~/.colaw, ~/.agents, and a bundled-root env setting
